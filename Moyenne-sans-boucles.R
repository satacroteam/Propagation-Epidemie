epidemie=function(n){ 
	...
	E=matrix(nrow=n,ncol=5000)				    #Matrice des états de chaque individu, à chaque instant t.
	E[,1]=1 										#On initialise tous les individus à l'état 1
	E[,2:5000]=0 								#On remplit les autres colones avec des 0
	IMN=sample(n,floor(n/10))				    #Un dizième de la pop est Immunisée naturelement
	E[IMN,1:5000]=3								#On applique l'état 3 à cette part de la population jusqu'à la fin.
	...
	while (2%in%E[,T]){ #on déroule la chaine tant qu'il y a encore des malades.	
		...
		PROCHE=which(apply(DIST,1,function(x) any(x<0.05))) #Retourne les indices des lignes de la matrice DIST 
		E[SA,T+1]=1										   #On applique l'état 1 à tous les sain, au temps T+1
		E[SA[PROCHE],T+1]=2                                #Applique l'état 2(infecté) aux individus sains correspondant aux indices de PROCHE		
		E[IN,T+1]=sapply(E[IN,T+1],function(x) x+sample(1:4,1,prob=Markov)) #Chaine de markov pour les individus infectés 
		E[which(E[,T+1]==3),(T+2):5000]=3 								    #On applique l'état 3 jusqu'à la fin pour les individus qui viennennt d'entrer dans cet état.
		E[which(E[,T+1]==4),(T+2):5000]=4   								    #On applique l'état 4 jusqu'à la fin pour les individus qui viennennt d'entrer dans cet état.
...
blabla=function(n,t){
	...
	plot(1:n,M[1:n],type='l',main="Moyenne du temps de disparition de l'épidémie")
}